...
def loudian_analyse(path,path_train):
    ...
    print(figure_data)
    figure_data=sorted(figure_data)
    print(figure_data)
    ...
    loudian1=[figure_data[0][1],figure_data[1][1],figure_data[2][1],figure_data[3][1],figure_data[4][1]]
    loudian2=[figure_data[0][0],figure_data[1][0],figure_data[2][0],figure_data[3][0],figure_data[4][0]]
    
    #print(loudian11,loudian22)
    #解决汉字乱码问题
    ...
    return data_number,max_loudian,min_loudian,figure_data



#第二个功能：进行预测建模，建立漏电电流和湿度、天气之间的关系，并根据明天晚上的天气来预测晚上8点到凌晨4点的漏电情况。
def loudian_decision_tree(data_train,path_test,figure_data):
...
def main():
    ...
    with open(result_path1, "a", newline='') as f:
      writer = csv.writer(f)
      writer.writerow(["rtu", "loop","data_number","min_loudian","max_loudian", "min0","max0","scale0","min1","max1","scale1","min2","max2","scale2","min3","max3","scale3","min4","max4","scale4"])
    ...
    for i in range(len(datasetlist)):
        ...
        data_number,max_loudian,min_loudian,figure_data=loudian_analyse(path,path_train)

        
        with open("E:/data_mining/loudian_problem/data_out/analyse_result.csv", "a", newline='') as f:
            writer = csv.writer(f)
            #writer.writerow(["URL", "rrr","predict", "score"])
            row = [[rtu_id,loop_id, data_number, min_loudian,max_loudian,figure_data[0][0],figure_data[0][1],figure_data[0][2],figure_data[1][0],figure_data[1][1],figure_data[1][2],figure_data[2][0],figure_data[2][1],figure_data[2][2],figure_data[3][0],figure_data[3][1],figure_data[3][2],figure_data[4][0],figure_data[4][1],figure_data[4][2]]]
            for r in row:
                writer.writerow(r)
        ...
        validation_accuracy,grade,min,max=loudian_decision_tree(train_path,test_path,figure_data) 
