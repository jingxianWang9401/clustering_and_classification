# -*- coding: utf-8 -*-
"""
Created on Fri Jul 10 09:09:24 2020
...
def fengxiantixing(path):
    ...
    kmeans=KMeans(n_clusters=6).fit(X_dataScale)#构建并训练模型
    #print('聚类结果为：',kmeans.labels_)
    
    quantity = pd.Series(kmeans.labels_).value_counts()
    print( "聚类后每个类别的样本数量\n", (quantity))
    print( "聚类后每个类别的样本数量\n", quantity[0],quantity[1],quantity[2],quantity[3],quantity[4],quantity[5])
    
    
    sum=quantity[0]+quantity[1]+quantity[2]+quantity[3]+quantity[4]+quantity[5]
    scale0=quantity[0]/sum
    scale1=quantity[1]/sum
    scale2=quantity[2]/sum
    scale3=quantity[3]/sum
    scale4=quantity[4]/sum
    scale5=quantity[5]/sum
    print('每个类别所占比例为：\n',scale0,'\n',scale1,'\n',scale2,'\n', scale3,'\n',scale4,'\n',scale5)
    '''
    a=quantity[0]
    b=quantity[1]
    ...
    loudian1=[max0_dianliu,max1_dianliu,max2_dianliu,max3_dianliu,max4_dianliu,max5_dianliu]
    loudian2=[min0_dianliu,min1_dianliu,min2_dianliu,min3_dianliu,min4_dianliu,min5_dianliu]
...
def data_preprocessing(path1,path2,loudian1,loudian2):
    ...
    def current_coding(x):
        if loudian2[0]<=x<=loudian1[0]:
            ...
            return 2
        elif loudian2[3]<=x<=loudian1[3]:
            return 3
        elif loudian2[4]<=x<=loudian1[4]:
            return 4
        elif loudian2[5]<=x<=loudian1[5]:
            return 5
...
path1='E:\data_mining\loudian_problem\data_hefei\dataset19.csv'

path2='E:/data_mining/loudian_problem/data_hefei/dataset19_yuchuli2.csv'
