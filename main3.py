# -*- coding: utf-8 -*-
"""
Created on Mon Sep 21 09:56:38 2020
...
def fengxiantixing(path):
    ...
    data=data[-data.rtu3.isin([0])]#删除漏电数据为0的所有行
    X=data.ix[:,7]
    ...
    if division0>15:
        ...
        df = df.drop(df[(df.rtu3 <=max0_dianliu) & (df.rtu3 >= min0_dianliu)].index)
        class_num=class_num-1
        
    if division1>15:
        #df = df.drop(df[min1_dianliu<df.rtu1.all() < max1_dianliu].index)
        df = df.drop(df[(df.rtu3 <= max1_dianliu) & (df.rtu3 >= min1_dianliu)].index)
        class_num=class_num-1

    if division2>15:
        #df = df.drop(df[min2_dianliu<df.rtu1.all() < max2_dianliu].index)
        df = df.drop(df[(df.rtu3 <= max2_dianliu) & (df.rtu3 >= min2_dianliu)].index)
        class_num=class_num-1

    if division3>15:
        #df = df.drop(df[min3_dianliu<df.rtu1.all()< max3_dianliu].index)
        df = df.drop(df[(df.rtu3 <= max3_dianliu) & (df.rtu3 >= min3_dianliu)].index)
        class_num=class_num-1

    if division4>15:
        #df = df.drop(df[min4_dianliu<df.rtu1.all()< max4_dianliu].index)
        df = df.drop(df[(df.rtu3 <= max4_dianliu) & (df.rtu3 >= min4_dianliu)].index)
        class_num=class_num-1
...
def data_preprocessing(path2,path3,loudian1,loudian2):
    ...
    data['rtu3']= data['rtu3'].apply(lambda x: current_coding(x))
...
path1='E:\data_mining\loudian_problem\data_hefei\dataset7.csv'

path2='E:\data_mining\loudian_problem\data_hefei_yuchuli\dataset7_dropyuchuli1.csv'

path3='E:/data_mining/loudian_problem/data_hefei_yuchuli/dataset7_finalyuchuli2.csv'

path_train='E:/data_mining/loudian_problem/data_hefei_yuchuli/dataset7_finaltrain.csv'

path_test='E:\data_mining\loudian_problem\data_hefei_yuchuli\dataset7_test.csv'
