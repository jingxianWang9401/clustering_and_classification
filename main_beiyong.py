# -*- coding: utf-8 -*-
"""
Created on Tue Jun 23 10:50:50 2020
...
def data_preprocessing(path1,path2,loudian1,loudian2):
    ...
    data['shebei5_huilu2']= data['shebei5_huilu2'].apply(lambda x: current_coding(x))
...
def loudian_decision_tree(path_train,path_test,loudian1,loudian2):
    ...
    if validation_accuracy<0.6:
        print('该线路漏电情况和天气等因素无关，请关注其他因素产生的漏电增加原因')
    elif validation_accuracy>=0.6:
        print('该线路漏电电流值和天气、温湿度等因素关系密切')
        #使用未知数据进行结果预测
        model=dtc.fit(X, y)
    ...
    '''
        time=data_test.iloc[:,1]
        loudian_dengji=real_result
        plt.plot(time, loudian_dengji, color='blue', label='class', linewidth=3.0)
        
        plt.margins(0)
        plt.subplots_adjust(bottom=0.10)
        plt.xlabel('time') #X轴标签
        plt.ylabel("class") #Y轴标签
        plt.ylim(0,3)
        plt.savefig('E:\data_mining\loudian_problem\loudianyuce.jpg')
        plt.show()  # 让图例生效
  '''      

        
    
path1='E:\data_mining\loudian_problem\data\dataset3.csv'

path2='E:/data_mining/loudian_problem/data/dataset3_yuchuli.csv'

path_test='E:\data_mining\loudian_problem\data\dataset3_test.csv'
